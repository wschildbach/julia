...
@testset "ranges" begin
    ...
    @testset "indexing" begin
        L32 = @inferred(linrange(Int32(1), Int32(4), 4))
        L64 = @inferred(linrange(Int64(1), Int64(4), 4))
        @test @inferred(L32[1]) === 1.0 && @inferred(L64[1]) === 1.0
        @test L32[2] == 2 && L64[2] == 2
        @test L32[3] == 3 && L64[3] == 3
        @test L32[4] == 4 && L64[4] == 4
        @test @inferred(linrange(1.0, 2.0, 2.0f0)) === linrange(1.0, 2.0, 2)
        @test @inferred(linrange(1.0, 2.0, 2))[1] === 1.0
        @test @inferred(linrange(1.0f0, 2.0f0, 2))[1] === 1.0f0
        @test @inferred(linrange(Float16(1.0), Float16(2.0), 2))[1] === Float16(1.0)
...
@testset "tricky floating-point ranges" begin
    for (start, step, stop, len) in ((1, 1, 3, 3), (0, 1, 3, 4),
        ...
        if len > 0
            l = linrange(start/10, stop/10, len)
            la = Vector(l)
    ...
    @test 1.0:1/49:27.0 == linrange(1.0,27.0,1275) == [49:1323;]./49
    @test isequal(1.0:1/49:27.0, linrange(1.0,27.0,1275))
    @test isequal(1.0:1/49:27.0, Vector(49:1323)./49)
    @test hash(1.0:1/49:27.0) == hash(linrange(1.0,27.0,1275)) == hash(Vector(49:1323)./49)
...
@testset "issue #7420 for type $T" for T = (Float32, Float64,), # BigFloat),
    ...
    r = strt:Δ:stop
    @test [r;] == vals
    @test [linrange(strt, stop, length(r));] == vals
    n = length(r)
    @test [r[1:n];] == [r;]
    @test [r[2:n];] == [r;][2:end]
...
@testset "issue #20373 (unliftable ranges with exact end points)" begin
    @test [3*0.05:0.05:0.2;]    == [linrange(3*0.05,0.2,2);]   == [3*0.05,0.2]
    @test [0.2:-0.05:3*0.05;]   == [linrange(0.2,3*0.05,2);]   == [0.2,3*0.05]
    @test [-3*0.05:-0.05:-0.2;] == [linrange(-3*0.05,-0.2,2);] == [-3*0.05,-0.2]
    @test [-0.2:0.05:-3*0.05;]  == [linrange(-0.2,-3*0.05,2);] == [-0.2,-3*0.05]
end
...
@testset "Inexact errors on 32 bit architectures. #22613" begin
    @test first(linrange(log(0.2), log(10.0), 10)) == log(0.2)
    @test last(linrange(log(0.2), log(10.0), 10)) == log(10.0)
    @test length(Base.floatrange(-3e9, 1.0, 1, 1.0)) == 1
end

@testset "linrange & ranges with very small endpoints for type $T" for T = (Float32, Float64)
    z = zero(T)
    u = eps(z)
    @test first(linrange(u,u,0)) == u
    @test last(linrange(u,u,0)) == u
    @test first(linrange(-u,u,0)) == -u
    @test last(linrange(-u,u,0)) == u
    @test [linrange(-u,u,0);] == []
    @test [linrange(-u,-u,1);] == [-u]
    @test [linrange(-u,u,2);] == [-u,u]
    @test [linrange(-u,u,3);] == [-u,0,u]
    @test first(linrange(-u,-u,0)) == -u
    @test last(linrange(-u,-u,0)) == -u
    @test first(linrange(u,-u,0)) == u
    @test last(linrange(u,-u,0)) == -u
    ...
    v = linrange(-u,u,12)
    @test length(v) == 12
    @test [-3u:u:3u;] == [linrange(-3u,3u,7);] == [-3:3;].*u
    @test [3u:-u:-3u;] == [linrange(3u,-3u,7);] == [3:-1:-3;].*u
end

@testset "linrange with very large endpoints for type $T" for T = (Float32, Float64)
    largeint = Int(min(maxintfloat(T), typemax(Int)))
    a = realmax()
    for i = 1:5
        @test [linrange(a,a,1);] == [a]
        @test [linrange(-a,-a,1);] == [-a]
        b = realmax()
        for j = 1:5
            @test [linrange(-a,b,0);] == []
            @test [linrange(-a,b,2);] == [-a,b]
            @test [linrange(-a,b,3);] == [-a,(b-a)/2,b]
            @test [linrange(a,-b,0);] == []
            @test [linrange(a,-b,2);] == [a,-b]
            @test [linrange(a,-b,3);] == [a,(a-b)/2,-b]
...
@testset "linrange with 1 or 0 elements (whose step length is NaN)" begin
    @test issorted(linrange(1,1,0))
    @test issorted(linrange(1,1,1))
end
# near-equal ranges
...
@testset "comparing and hashing ranges" begin
    Rs = AbstractRange[1:1, 1:1:1, 1:2, 1:1:2,
                       ...
                       linrange(0, 1, 20), map(Float32, linrange(0, 1, 20))]
    for r in Rs
        local r
        ar = Vector(r)
        @test r == ar
...
@testset "issue #7114" begin
    ...
    let r = linrange(1/3, 5/7, 6)
        @test length(r) == 6
        @test r[1] == 1/3
        @test abs(r[end] - 5/7) <= eps(5/7)
    end

    let r = linrange(0.25, 0.25, 1)
        @test length(r) == 1
        @test_throws ArgumentError linrange(0.25,0.5,1)
    end
end
...
@testset "Issue #11245" begin
    io = IOBuffer()
    show(io, linrange(1, 2, 3))
    str = String(take!(io))
#    @test str == "linrange(1.0,2.0,3)"
...
@testset "stringmime/repr" begin
    # stringmime/show should display the range or linrange nicely
    ...
    @test stringmime("text/plain", linrange(1,5,7)) == "1.0:0.6666666666666666:5.0"
    @test stringmime("text/plain", LinRange{Float64}(1,5,7)) == "7-element LinRange{Float64}:\n 1.0,1.66667,2.33333,3.0,3.66667,4.33333,5.0"
    @test repr(linrange(1,5,7)) == "1.0:0.6666666666666666:5.0"
    @test repr(LinRange{Float64}(1,5,7)) == "linrange(1.0,5.0,7)"
    ...
    @test replstrmime(linrange(0,100, 10000)) == "0.0:0.010001000100010001:100.0"
...
@testset "Issue 11049 and related" begin
    @test promote(linrange(0f0, 1f0, 3), linrange(0., 5., 2)) ===
        (linrange(0., 1., 3), linrange(0., 5., 2))
    @test convert(LinRange{Float64}, linrange(0., 1., 3)) === LinRange(0., 1., 3)
    @test convert(LinRange{Float64}, linrange(0f0, 1f0, 3)) === LinRange(0., 1., 3)

    @test promote(linrange(0., 1., 3), 0:5) === (linrange(0., 1., 3),
                                                 linrange(0., 5., 6))
    ...
    test_range_index(linrange(0.1, 0.3, 3), 1:2)
    test_range_index(linrange(0.1, 0.3, 3), 1:0)
    test_range_index(linrange(1.0, 1.0, 1), 1:1)
    test_range_index(linrange(1.0, 1.0, 1), 1:0)
    test_range_index(linrange(1.0, 2.0, 0), 1:0)

    function test_linrange_identity(r::AbstractRange{T}, mr) where T
        @test -r == mr
    ...
    test_linrange_identity(linrange(1.0, 27.0, 10), linrange(-1.0, -27.0, 10))
    test_linrange_identity(linrange(1f0, 27f0, 10), linrange(-1f0, -27f0, 10))

    test_linrange_identity(linrange(1.0, 27.0, 0), linrange(-1.0, -27.0, 0))
    test_linrange_identity(linrange(1f0, 27f0, 0), linrange(-1f0, -27f0, 0))

    test_linrange_identity(linrange(1.0, 1.0, 1), linrange(-1.0, -1.0, 1))
    test_linrange_identity(linrange(1f0, 1f0, 1), linrange(-1f0, -1f0, 1))

    @test reverse(linrange(1.0, 27.0, 1275)) == linrange(27.0, 1.0, 1275)
    @test [reverse(linrange(1.0, 27.0, 1275));] ==
        reverse([linrange(1.0, 27.0, 1275);])
end
@testset "PR 12200 and related" begin
    for _r in (1:2:100, 1:100, 1f0:2f0:100f0, 1.0:2.0:100.0,
               linrange(1, 100, 10), linrange(1f0, 100f0, 10))
        float_r = float(_r)
        big_r = big.(_r)
    ...
    @test_throws DimensionMismatch linrange(1.,5.,5) + linrange(1.,5.,6)
    @test_throws DimensionMismatch linrange(1.,5.,5) - linrange(1.,5.,6)
    @test_throws DimensionMismatch linrange(1.,5.,5) .* linrange(1.,5.,6)
    @test_throws DimensionMismatch linrange(1.,5.,5) ./ linrange(1.,5.,6)
    ...
    test_range_sum_diff(1.:5., 0.:2.:8., 1.:3.:13., 1.:-1.:-3.)
    test_range_sum_diff(linrange(1.,5.,5), linrange(0.,-4.,5),
                        linrange(1.,1.,5), linrange(1.,9.,5))

    test_range_sum_diff(1:5, 0.:2.:8., 1.:3.:13., 1.:-1.:-3.)
    test_range_sum_diff(1:5, linrange(0, 8, 5),
                        linrange(1, 13, 5), linrange(1, -3, 5))
    test_range_sum_diff(1.:5., linrange(0, 8, 5),
                        linrange(1, 13, 5), linrange(1, -3, 5))
end
# Issue #12388
...
@testset "linrange of other types" begin
    let r = linrange(0, 3//10, 4)
...
@testset "issue #23178" begin
    r = linrange(Float16(0.1094), Float16(0.9697), 300)
...
@testset "issue #14420" begin
    for r in (linrange(0.10000000000000045, 1, 50), 0.10000000000000045:(1-0.10000000000000045)/49:1)
        local r
